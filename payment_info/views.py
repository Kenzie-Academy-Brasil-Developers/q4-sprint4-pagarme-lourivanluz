...
class PaymentesInfoView(generics.ListCreateAPIView):
    ...
    def create(self, request:Request):
        ...
        if not card_list:
            ...
            response['card_number'] = format_card_number(response['card_number'])
            response['customer'] = user.id

            return Response(response,HTTP_201_CREATED)
        return Response({"error": ["This card is already registered for this user"]},HTTP_422_UNPROCESSABLE_ENTITY)
        
    def list(self, request:Request):
        user:Users = request.user
        serializer = PaymentInfoSerializer(PaymentInfo.objects.filter(customer_id=user.id).all(), many=True)
        response = []
